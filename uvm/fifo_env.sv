...
class fifo_env #(parameter int DATA_WIDTH = 8, 
  ...
  function new(string name, uvm_component parent);
    ...
    `uvm_info(get_type_name(), "Environment instantiated", UVM_HIGH);
  endfunction
  ...
  function void build_phase(uvm_phase phase);
    ...
    `uvm_info(get_type_name(), "Environment built", UVM_HIGH);
  endfunction
  ...
  function void connect_phase(uvm_phase phase);
    ...
    m_agent.monitor.mon_analysis_port.connect(m_scoreboard.ap_imp);
      `uvm_info(get_type_name(), "Environment connected", UVM_HIGH);
  endfunction
